<?php
    function addBlogger ($blogname, $username, $password, $blogurl, $personname) {
        $sql = "INSERT INTO `blogs` (`blogname` ,`username` ,`password`, `blogurl`, `personname`)
                VALUES ('".$blogname."',  '".$username."',  '".$password."', '".$blogurl."', '".$personname."');";
        if (!mysql_query($sql)) {
            return false;
        }
    ...
    function updateBlogTitle($blog_id, $newtitle) {
        $sql = "UPDATE `blogs` SET  `blogname` =  '".$newtitle."' WHERE  `blogs`.`id` =".$blog_id.";";
        if (mysql_query($sql)) {
            return true;
        }
